        ...
        <div class="grid-container">
            <div class="info-column">
                <div class="hosting-info">
                    <h2>{{$vps->name}}</h2><hr>
                    <ul class="supp">
                        <li><strong>Dung Lượng: {{$vps->data_Hosting}} GB NVME</strong></li>
                           @php
                                  $lines = explode("\n", $vps->mo_ta);
                           @endphp
                           ...
                           @endforeach
                           <li><strong>Băng Thông: </strong>{{$vps->bandwith}}MB/S</li>
                        </ul>
                </div>
            </div>
            <input id="price" type="hidden" value="{{$vps->GiaTien}}">
            ...
            <div class="total-column">
                <div class="total-price">
                    ...
                    <br> <br>
                    <form method="POST" action="{{ route('payment_vnpay',['type'=>$vps->type_product]) }}">
                        @csrf
                        <input type="hidden" id="totalpay" name="totalpay" value="">
                        <button type="submit" class="read_more read_more_button">Thanh Toán</button>
                    </form>
                    <br>
                    <form method="POST" action="{{ route('addcart', [ 'type'=>$vps->type_product, 'id' => $vps->ID]) }}">
                        @csrf
